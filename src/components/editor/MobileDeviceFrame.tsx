...
interface MobileDeviceFrameProps {
  ...
  deviceType?: 'iphone' | 'desktop';
  onDeviceTypeChange?: (type: 'iphone' | 'desktop') => void;
}
...
export const MobileDeviceFrame: React.FC<MobileDeviceFrameProps> = ({
  ...
  const [baseUrl, setBaseUrl] = useState('https://weddora.web.id');
        ...
        <div className={`px-4 py-2.5 flex items-center justify-between border-b text-xs transition-colors ${
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`px-3 py-1 rounded-lg font-mono text-[11px] border ${
              ...
              {baseUrl}/demo/luxury-gold-marble (Mode Fullscreen Website)
            </span>
          </div>
  ...
  return (
    <>
      {/* MOBILE NATIVE VIEW (< md): Full width & height without simulated outer bezel */}
      <div className="md:hidden w-full h-full flex flex-col overflow-hidden bg-slate-950">
        <div className="flex-1 overflow-y-auto scrollbar-thin relative [transform:translateZ(0)] w-full h-full">
          {children}
        </div>
      </div>

      {/* DESKTOP DEVICE MOCKUP (>= md): Elegant iPhone 15 Pro frame */}
      <div className="hidden md:flex flex-col items-center justify-center h-full w-full overflow-hidden p-2 select-none">
        ...
        <div className={`flex items-center justify-between w-full max-w-[380px] mb-2 px-3 py-2 rounded-xl border text-xs shadow-md transition-colors ${
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="gold-metallic-bg text-slate-950 px-2.5 py-0.5 rounded-full font-extrabold text-[10px] uppercase shadow">
              {deviceType === 'iphone' ? 'iPhone 15 Pro' : 'Website Desktop'}
            </span>
          </div>
        ...
        <div className="relative w-[360px] h-[680px] max-h-[calc(100vh-140px)] bg-slate-900 rounded-[48px] p-3 shadow-2xl border-4 border-gold-500/50 shadow-black flex flex-col shrink-0">
